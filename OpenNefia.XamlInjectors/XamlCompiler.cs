    ...
    public partial class XamlCompiler
    {
            ...
            string projectDirectory, string output, string strongNameKey)
        {
            var typeSystem = new CecilTypeSystem(references
                .Where(r => !r.ToLowerInvariant().EndsWith("opennefia.build.tasks.dll"))
                .Concat(new[] { input }), input);

            var asm = typeSystem.TargetAssemblyDefinition;

            if (asm.MainModule.GetType("CompiledOpenNefiaXaml", "XamlIlContext") != null)
            {
        ...
        static bool? CompileCore(IBuildEngine engine, CecilTypeSystem typeSystem)
        {
            ...
            {
                var typeDef = new TypeDefinition("CompiledOpenNefiaXaml", "!" + group.Name, TypeAttributes.Class,
                    asm.MainModule.TypeSystem.Object);
                ...
                foreach (var res in group.Resources.Where(CheckXamlName))
                {
                    try
                    {
                        ...
                        compiler.Compile(parsed, contextClass,
                          ...
                          builder.DefineSubType(transformerconfig.WellKnownTypes.Object, "NamespaceInfo:" + res.Name,
                              true),
        ...
        {
            var nameScopeType = typeSystem.FindType("OpenNefia.Core.UserInterface.XAML.NameScope");
