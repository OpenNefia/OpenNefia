...
using OpenNefia.XamlInjectors.Utilities;
        ...
        class AvaloniaResources : IResourceGroup
        {
            ...
            public AvaloniaResources(AssemblyDefinition asm, string projectDir)
            {
                ...
                _embedded = ((EmbeddedResource)asm.MainModule.Resources.FirstOrDefault(r =>
                    r.ResourceType == ResourceType.Embedded && r.Name == "!EmbeddedResources"));
                if (_embedded == null)
                    return;
                using (var stream = _embedded.GetResourceStream())
                {
                    var br = new BinaryReader(stream);
                    var index = OpenNefiaResourcesIndexReaderWriter.Read(new MemoryStream(br.ReadBytes(br.ReadInt32())));
            ...
            public void Save()
            {
                ...
                _embedded = new EmbeddedResource("!EmbeddedResources", ManifestResourceAttributes.Public,
                    OpenNefiaResourcesIndexReaderWriter.Create(_resources.ToDictionary(x => x.Key,
