...
using OpenNefia.XamlInjectors.Compiler;
...
namespace OpenNefia.Build.Tasks
{
    public static partial class XamlCompilerTaskExecutor
    {
            ...
            , bool debuggerLaunch = false)
        {
            ...
            var clrPropertiesDef = new TypeDefinition("CompiledOpenNefiaXaml", "XamlIlHelpers",
                TypeAttributes.Class, asm.MainModule.TypeSystem.Object);
            asm.MainModule.Types.Add(clrPropertiesDef);
            var indexerAccessorClosure = new TypeDefinition("CompiledOpenNefiaXaml", "!IndexerAccessorFactoryClosure",
                TypeAttributes.Class, asm.MainModule.TypeSystem.Object);
            asm.MainModule.Types.Add(indexerAccessorClosure);
            var trampolineBuilder = new TypeDefinition("CompiledOpenNefiaXaml", "XamlIlTrampolines",
                TypeAttributes.Class, asm.MainModule.TypeSystem.Object);
            asm.MainModule.Types.Add(trampolineBuilder);

            var (xamlLanguage , emitConfig) = OpenNefiaXamlIlLanguage.Configure(typeSystem);
            var compilerConfig = new ONXamlIlCompilerConfiguration(typeSystem,
                typeSystem.TargetAssembly,
                xamlLanguage,
                XamlXmlnsMappings.Resolve(typeSystem, xamlLanguage),
                OpenNefiaXamlIlLanguage.CustomValueConverter,
                new XamlIlTrampolineBuilder(typeSystem.CreateTypeBuilder(trampolineBuilder)),
                new DeterministicIdGenerator());


            var contextDef = new TypeDefinition("CompiledOpenNefiaXaml", "XamlIlContext", 
                TypeAttributes.Class, asm.MainModule.TypeSystem.Object);
            ...
            var compiler = new OpenNefiaXamlILCompiler(compilerConfig, emitConfig, contextClass) { EnableIlVerification = verifyIl };
            ...
            var loaderDispatcherDef = new TypeDefinition("CompiledOpenNefiaXaml", "!XamlLoader",
                TypeAttributes.Class, asm.MainModule.TypeSystem.Object);
            ...
            {
                var typeDef = new TypeDefinition("CompiledOpenNefiaXaml", "!"+ group.Name,
                    TypeAttributes.Class, asm.MainModule.TypeSystem.Object);
                ...
                foreach (var res in group.Resources.Where(CheckXamlName).OrderBy(x=>x.FilePath.ToLowerInvariant()))
                {
                    try
                    {
                        ...
                        // WispControl.Class property
                        var classDirective = initialRoot.Children.OfType<XamlAstXamlPropertyValueNode>()
                            .FirstOrDefault(n => n.Property is XamlAstNamePropertyReference propRef && propRef.Name == "Class");
                        ...
                        if (classDirective != null)
                        {
                            if (classDirective.Values.Count != 1 || !(classDirective.Values[0] is XamlAstTextNode tn))
                                throw new XamlParseException("'Class' property should have a string value", classDirective);
                        ...
                        compiler.Compile(parsed, contextClass,
                            ...
                            compilerConfig.TypeMappings.XmlNamespaceInfoProvider == null ? null : 
                            ...
                            if (!foundXamlLoader)
                            {
                                ...
                                else
                                {
                                    throw new InvalidProgramException(
                                        $"No call to OpenNefiaXamlLoaer.Load(this) call found anywhere in the type {classType.FullName} and type seems to have custom constructors.");
                                }
                            }
                    ...
                    catch (Exception e)
                    {
                        ...
                        engine.LogErrorEvent(new BuildErrorEventArgs("OpenNefia", "XAMLIL", res.FilePath,
