...
namespace OpenNefia.XamlNameGenerator
{
    ...
    {
        private const string IHasXamlReferencesType = "OpenNefia.Core.UserInterface.IHasXamlReferences";
        private const string AttributeName = "OpenNefia.Core.UserInterface.GenerateTypedNameReferencesAttribute";
        private const string AttributeFile = "GenerateTypedNameReferencesAttribute";

        private const string AttributeCode = @"// <auto-generated />
using System;
namespace OpenNefia.Core.AutoGenerated
{
...
";
        ...
        class NameVisitor : IXamlAstVisitor
        {
            ...
            private bool IsControl(IXamlType type) => type.FullName != "System.Object" &&
                                                      (type.FullName == "OpenNefia.Core.UI.Element.UiElement" ||
...
using OpenNefia.Core.UserInterface;
using OpenNefia.Core.UserInterface.Controls;
namespace {nameSpace}
{{
...
";
        ...
        public void Execute(GeneratorExecutionContext context)
        {
            ...
            var allowedNameGenerator = context
                .GetMSBuildProperty("OpenNefiaXamlNameGenerator", "true")
                .Equals("true", StringComparison.OrdinalIgnoreCase);

            if (!allowedNameGenerator && comp.GetTypeByMetadataName(AttributeName) == null)
                    ...
                    context.ReportDiagnostic(
                        Diagnostic.Create(
                            new DiagnosticDescriptor(
                                "OXN0001",
                                $"Unable to discover the relevant OpenNefia XAML file for {typeSymbol}.",
                                "Unable to discover the relevant OpenNefia XAML file " +
            ...
            CSharpCompilation comp, NameReferenceSyntaxReceiver receiver)
        {
            ...
            foreach (var candidateClass in receiver.CandidateClasses)
            {
                ...
                if (InheritsFrom(typeSymbol, IHasXamlReferencesType) == false)
                {
                    continue;
                }
                ...
                {
                    // Check if control is annotated with [GenerateTypedNameReferencesAttribute]
                ...
                else
                {
                    var missingPartialKeywordMessage =
                        $"The type {typeSymbol.Name} should be declared with the 'partial' keyword as ";

                    if (allowedNameGenerator == false)
                    {
                        missingPartialKeywordMessage += "it is annotated with the [GenerateTypedNameReferences] attribute";
                    }
                    else
                    {
                        missingPartialKeywordMessage += $"it derives from IHasXamlReferences.";
                    };
        ...
        public static bool InheritsFrom(INamedTypeSymbol symbol, string typeName)
        {
            while (true)
            {
