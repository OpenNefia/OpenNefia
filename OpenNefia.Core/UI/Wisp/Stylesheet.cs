...
namespace OpenNefia.Core.UI.Wisp
{
    ...
    public sealed class Stylesheet
    {
        ...
        public Stylesheet(IReadOnlyList<StyleRule> rules)
        {
            ...
            var order = 0;
            var typeControl = typeof(WispControl);
    ...
    public static class StylesheetHelpers
    {
        ...
        /// <seealso cref="SelectorElement"/>
        public static MutableSelectorElement Element<T>() where T : WispControl
        {
    ...
    {
        public abstract bool Matches(WispControl control);
    ...
    public sealed class SelectorElement : Selector
    {
            ...
            IEnumerable<string>? pseudoClass)
        {
            if (elementType != null)
            {
                if (!typeof(WispControl).IsAssignableFrom(elementType))
                {
                    throw new ArgumentException($"elementType must inherit {nameof(WispControl)}.", nameof(elementType));
                }

                if (elementType == typeof(WispControl))
                {
                    throw new ArgumentException($"elementType may not be {nameof(WispControl)} itself.", nameof(elementType));
                }
            }
        ...
        public override bool Matches(WispControl control)
        {
    ...
    public sealed class SelectorChild : Selector
    {
        ...
        public override bool Matches(WispControl control)
        {
            ...
            return Parent.Matches(control.WispParent) && Child.Matches(control);
        }
